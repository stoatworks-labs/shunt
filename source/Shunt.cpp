...
static_assert( kMaxShapes == 64, "Shaders.cpp declares Xform[64] and Tint[64] -- keep them in step" );
...
QueueParams ShuntPlugin::CurrentQueue( int width, int height ) const
{
	...
	q.opacity = Clamp01( params[ PT_OPACITY ] ) * ( overInput ? Clamp01( params[ PT_MIX ] ) : 1.0f );
